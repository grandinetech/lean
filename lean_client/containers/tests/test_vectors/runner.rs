...
impl TestRunner {
    ...
    pub fn run_single_block_with_slot_gap_tests<P: AsRef<Path>>(
        ...
        if let Some(ref blocks) = test_case.blocks {
            ...
            for (idx, block) in blocks.iter().enumerate() {
                println!(
                    "\nProcessing block {}: slot {:?} (gap from slot {:?})",
                    idx + 1,
                    block.slot,
                    state.slot
                );
                ...
                match result {
                    Ok(new_state) => {
                        ...
                        println!("  ✓ State root matches: {:?}", computed_state_root);
                        println!(
                            "  ✓ Block {} processed successfully (with {} empty slots)",
                            idx + 1,
                            block.slot.0 - test_case.pre.slot.0 - idx as u64
                        );
                    }
                    Err(e) => {
                        return Err(format!("Block {} processing failed: {:?}", idx + 1, e).into());
