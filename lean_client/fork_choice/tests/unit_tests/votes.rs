...
fn test_competing_votes_different_blocks() {
    ...
    store.blocks.insert(block_a_root, SignedBlockWithAttestation {
    ...
    store.blocks.insert(block_b_root, SignedBlockWithAttestation {
        message: BlockWithAttestation {
            block: block_b,
            proposer_attestation: Default::default(),
        },
    ...
    let mut attestations = HashMap::new();
    for i in 0..3 {
        attestations.insert(ValidatorIndex(i), create_signed_attestation(
            i, 1, block_a_root, 1, genesis_root, 0, genesis_root, 0,
        ));
    }
    for i in 3..5 {
        attestations.insert(ValidatorIndex(i), create_signed_attestation(
            i, 1, block_b_root, 1, genesis_root, 0, genesis_root, 0,
        ));
    }
...
fn test_vote_weight_accumulation() {
    ...
    store.blocks.insert(block1_root, SignedBlockWithAttestation {
        message: BlockWithAttestation { block: block1, proposer_attestation: Default::default() },
        signature: Default::default(),
    });
    store.blocks.insert(block2_root, SignedBlockWithAttestation {
        message: BlockWithAttestation { block: block2, proposer_attestation: Default::default() },
        signature: Default::default(),
    });
    
    // Vote for block2 - should accumulate to block1 as well
    let mut attestations = HashMap::new();
    attestations.insert(ValidatorIndex(0), create_signed_attestation(
        0, 2, block2_root, 2, genesis_root, 0, genesis_root, 0,
    ));
...
fn test_duplicate_vote_uses_latest() {
    ...
    // Insert a vote
    attestations.insert(ValidatorIndex(0), create_signed_attestation(
        0, 1, genesis_root, 0, genesis_root, 0, genesis_root, 0,
    ));
    
    // "Update" with same validator - only latest is kept
    attestations.insert(ValidatorIndex(0), create_signed_attestation(
        0, 2, genesis_root, 0, genesis_root, 0, genesis_root, 0,
    ));
...
fn test_vote_for_unknown_block_ignored() {
    ...
    let mut attestations = HashMap::new();
    attestations.insert(ValidatorIndex(0), create_signed_attestation(
        0, 1, unknown_root, 1, genesis_root, 0, genesis_root, 0,
    ));
