...
use fork_choice::store::{get_forkchoice_store, get_fork_choice_head, Store};
use ssz::SszHash;
...
fn add_block(store: &mut Store, slot: u64, parent_root: Bytes32, proposer: u64) -> Bytes32 {
    ...
    store.blocks.insert(block_root, SignedBlockWithAttestation {
        message: BlockWithAttestation {
            block,
            proposer_attestation: Default::default(),
        },
...
fn test_basic_slot_transition() {
    ...
    let checkpoint = Checkpoint { root: block3_root, slot: Slot(3) };
    let genesis_checkpoint = Checkpoint { root: genesis_root, slot: Slot(0) };
    
    attestations.insert(ValidatorIndex(0), create_attestation(
        0, 3, checkpoint.clone(), checkpoint.clone(), genesis_checkpoint.clone()
    ));
...
fn test_attestation_processing() {
    ...
    // Create a block
    let block1_root = add_block(&mut store, 1, genesis_root, 0);
    let block1_checkpoint = Checkpoint { root: block1_root, slot: Slot(1) };
    
    // Process attestations from multiple validators
    let mut attestations = HashMap::new();
    for i in 0..5 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 1, block1_checkpoint.clone(), block1_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
...
fn test_multiple_attestations() {
    ...
    let block3_checkpoint = Checkpoint { root: block3_root, slot: Slot(3) };
    
    // All validators attest to block3
    let mut attestations = HashMap::new();
    for i in 0..10 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 3, block3_checkpoint.clone(), block3_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
...
fn test_fork_choice_with_competing_blocks() {
    ...
    let fork_a_checkpoint = Checkpoint { root: fork_a_root, slot: Slot(1) };
    let fork_b_checkpoint = Checkpoint { root: fork_b_root, slot: Slot(1) };
    
    // 6 validators vote for fork A
    let mut attestations = HashMap::new();
    for i in 0..6 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 1, fork_a_checkpoint.clone(), fork_a_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
    ...
    for i in 6..10 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 1, fork_b_checkpoint.clone(), fork_b_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
...
fn test_finality_prevents_reorg() {
    ...
    store.latest_finalized = Checkpoint { root: block1_root, slot: Slot(1) };
    ...
    let block2_checkpoint = Checkpoint { root: block2_root, slot: Slot(2) };
    let competing_checkpoint = Checkpoint { root: competing_root, slot: Slot(1) };
    
    // More votes for competing fork
    let mut attestations = HashMap::new();
    for i in 0..7 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 1, competing_checkpoint.clone(), competing_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
    for i in 7..10 {
        attestations.insert(ValidatorIndex(i), create_attestation(
            i, 2, block2_checkpoint.clone(), block2_checkpoint.clone(), genesis_checkpoint.clone()
        ));
    }
...
fn test_attestation_from_future_slot() {
    ...
    let mut attestations = HashMap::new();
    attestations.insert(ValidatorIndex(0), create_attestation(
        0, 100, block1_checkpoint.clone(), block1_checkpoint.clone(), genesis_checkpoint.clone()
    ));
